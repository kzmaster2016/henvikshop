...
class CartLogic extends RelationModel
{
    ...
    {       
        ...
        if($catr_count >= 20) 
            return array('status'=>-9,'msg'=>'Shopping cart can only put up to 20 kinds of goods','result'=>'');            
        
        if(!empty($specGoodsPriceList) && empty($goods_spec)) // 有商品规格 但是前台没有传递过来
            return array('status'=>-1,'msg'=>'need send the goods specifications','result'=>'');                        
        if($goods_num <= 0) 
            return array('status'=>-2,'msg'=>'The number of purchases can not be 0','result'=>'');            
        if(empty($goods))
            return array('status'=>-3,'msg'=>'The Goods does not exist','result'=>'');            
        if(($goods['store_count'] < $goods_num))
            return array('status'=>-4,'msg'=>'Lack of inventory','result'=>'');   

        // if($goods['prom_type'] > 0 && $user_id == 0) //可以先加入购物车，在购物车中判断
        //     return array('status'=>-101,'msg'=>'This Goods is on sales promotion,you need login first','result'=>'');
        ...
        if($goods['prom_type'] == 1) 
        {
            ...
            if($flash_sale){
                ...
                {  
                    $cart_goods_num && $error_msg = "Your Cart has $cart_goods_num pieces!";
                    return array('status'=>-4,'msg'=>"Each one could only buy {$flash_sale['buy_limit']} pieces $error_msg",'result'=>'');
                }                        
                // 如果剩余数量 不足 限购数量, 就只能买剩余数量
                if(($flash_sale['goods_num'] - $flash_sale['buy_num']) < $flash_sale['buy_limit'])
                    return array('status'=>-4,'msg'=>"Lack of inventory, you could only buy".($flash_sale['goods_num'] - $flash_sale['buy_num'])."pieces.",'result'=>'');                    
            }
        }                
       ...
       {          
           // 如果购物车的已有数量加上 这次要购买的数量  大于  库存输  则不再增加数量
            ...
            return array('status'=>1,'msg'=>'Add to Cart Successful!','result'=>$cart_count);
       }
       else
       {         
            ...
            $cart_count = cart_goods_num($user_id,$session_id); // 查找购物车数量 
            return array('status'=>-5,'msg'=>'Add to Cart failed','result'=>$cart_count);        
    }
    ...
    {                   
        ...
        foreach ($cartList as $k=>$val){
        	...
        	$cartList[$k]['goods_img']  = getGoodImg($val['goods_id']); // 最多可购买的库存数量        	
                $anum += $val['goods_num'];
    ...
    public function getCouponMoneyByCode($couponCode,$order_momey)
    {
        ...
        if(empty($couponlist)) 
            return array('status'=>-9,'msg'=>'Coupon codes is not exist.','result'=>'');
        $coupon = M('Coupon')->where("id = {$couponlist['cid']}")->find(); // 获取优惠券类型表
        if(time() > $coupon['use_end_time'])  
            return array('status'=>-10,'msg'=>'Coupons has expired.','result'=>'');
        if($order_momey < $coupon['condition'])
            return array('status'=>-11,'msg'=>'The amount is not enough for the coupon.','result'=>'');
        if($couponlist['order_id'] > 0)
            return array('status'=>-12,'msg'=>'Coupons has been used.','result'=>'');
    ...
    public function addOrder($user_id,$address_id,$shipping_code='',$invoice_title,$coupon_id = 0,$car_price)
    {
        ...
        if($order_count >= 50) 
            return array('status'=>-9,'msg'=>'Only 50 orders for a day','result'=>'');            
