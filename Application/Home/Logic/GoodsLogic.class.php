...
class GoodsLogic extends RelationModel
{
    ...
    {
        if (!is_numeric($user_id) || $user_id <= 0) return array('status' => -1, 'msg' => 'After log in,you can collect', 'result' => array());
        ...
        if ($count > 0) return array('status' => -3, 'msg' => 'The Goods have been collected', 'result' => array());
        M('GoodsCollect')->add(array('goods_id' => $goods_id, 'user_id' => $user_id, 'add_time' => time()));
        return array('status' => 1, 'msg' => 'Collection successful! Please go to the user center to see', 'result' => array());
    }
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if ($i == 0)
                $parr[] = array('value' => "{$end} or more", 'href' => urldecode(U("Goods/$action", $filter_param, '')));
            elseif ($i == ($c - 1))
                $parr[] = array('value' => "{$end} or below", 'href' => urldecode(U("Goods/$action", $filter_param, '')));
            else
    ...
    function get_filter_menu($filter_param, $action)
    {
        ...
        // 价格
        if (!empty($filter_param['price'])) {
            $price_menu['text'] = "price:" . $filter_param['price'];
