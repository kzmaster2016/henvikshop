...
function get_city(t){
    ...
    $.ajax({
        ...
        success: function(v) {
            v = '<option value="0">Province or City</option>'+ v;          
            $('#city').empty().html(v);
        }
    });
}
...
function get_area(t){
    ...
    $.ajax({
        ...
        success: function(v) {
            v = '<option>Town/Village</option>'+ v;
...
// 倒计时
function GetRTimeDetail(end_time){
  // var EndTime= new Date('2016/05/1 10:00:00'); //截止时间 前端路上 http://www.51xuediannao.com/qd63/
    var EndTime= new Date(end_time); //截止时间 前端路上 http://www.51xuediannao.com/qd63/
    var NowTime = new Date();
    var t =EndTime.getTime() - NowTime.getTime();
...
*计算价格优惠比例
*/
function calPriceScale(origin_price,market_price,shop_price,promotion_price){
        var origin_scale;
        if (promotion_price) {
            if (origin_price>0){
                origin_scale=(origin_price-promotion_price)/origin_price;
            }else if(market_price>promotion_price){
                origin_scale=(market_price-promotion_price)/market_price;
            }else{
                return false;
            }
        }else{
            if (origin_price>0){
                origin_scale=(origin_price-shop_price)/origin_price;
            }else if(market_price>shop_price){
                origin_scale=(market_price-shop_price)/market_price;
            }else{
                return false;
            }
        }
        origin_scale=Math.ceil(origin_scale*100);
        return origin_scale;       
    }
